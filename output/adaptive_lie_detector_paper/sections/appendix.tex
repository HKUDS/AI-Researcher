...
EXP-I (matched) & Matched autonomous (3B+14B+7B) & 50--200 & \S\ref{sec:autonomous_matched} & Syco: 68.5\% (3B), 83\% (14B), 72\% (70B); P/FB: 54/56\% (3B, chance); 68/60\% (14B, excl.\ chance, fail Bonf.); all $n{=}200$ \\
...
\begin{table}[h]
\centering
\caption{\textbf{Primary claim-supporting numbers.} One canonical value per claim. ``Extractor'' identifies the LLM producing the 5 behavioral features; ``Regex'' indicates the extractor-independent refusal-count rule (fixed $k\!\geq\!1$). \textbf{Validity}: L-indep = level-independent (extractor-independent, ICC-unaffected); L-dep = level-dependent (read with §\ref{sec:icc_caveat}).}
\label{tab:headlines_primary}
\label{tab:headlines}
\begin{small}
\begin{tabular}{lrrlllll}
\toprule
\textbf{Finding} & \textbf{Value} & \textbf{n} & \textbf{Regime} & \textbf{Extractor} & \textbf{Ref.} & \textbf{Validity} \\
\midrule
Instructed LOO (7-model range) & 93.9--100\% & 92--100 & Instructed & Haiku & App.~\ref{app:instructed_results} & L-dep ($\alpha{<}0.27$) \\
Equalized rule avg (pooled-LOO, $k\!=\!1$) & 80.1\% & 689 & Equalized & Regex & Tab.~\ref{tab:refusal_only} & L-indep \\
Cross-family pipeline (7-target, Mistral L3) & 64.7\% & 689 & Equalized & Mistral L3 & Tab.~\ref{tab:cross_family_panel} & L-dep ($\alpha{=}0.61$) \\
Instructed-matched $\Delta$LOO (clean $\geq$14B) & $+$7.5 to $+$15 pp & 99--100 & Decomp & Haiku & \S\ref{sec:prompt_equalized} & L-dep \\
Sycophancy transfer (3B/14B/70B, rule) & 68.5/83/72\% & 200/200/50 & Semi-auto & Regex & Tab.~\ref{tab:autonomous_matched} & L-indep \\
Persona/false-belief (3B; 14B; rule) & 60--66\% & 50 each & Autonomous & Regex & Tab.~\ref{tab:autonomous_matched} & L-indep \\
\bottomrule
\end{tabular}
\end{small}
\end{table}
...
\begin{table}[h]
\centering
\caption{\textbf{Diagnostic numbers.} Adversarial robustness and construct-validity.}
...
\begin{table}[h]
\centering
\caption{Refusal-count-only LOO vs.\ full pipelines on equalized data (EXP-J). Pooled 7-model dataset (689 trials); fixed threshold $k=1$.}
...
\begin{table}[h]
\centering
\caption{Matched-format autonomous deception detection (EXP-I). Refusal-count LOO accuracy and effect sizes. $^\star$: $n=50$ pilot, treat as preliminary.}
...
Sycophancy defense        & Llama 3.2 3B & 200 & 68.5\% & [61.8--74.5\%] & 1.0 \\
...
Sycophancy defense        & \textbf{Qwen 2.5 14B} & 200 & \textbf{83.0\%} & [77.2--87.6\%] & \textbf{1.52} \\
\midrule
...
Llama~4~Maverick (\texttt{us.meta.llama4-maverick-17b-instruct-v1:0}, AWS Bedrock) was evaluated under the same prompt-equalized protocol as all other targets ($n\!=\!100$, Haiku interrogator/extractor, cross-family). Results will be integrated here upon experiment completion. The experiment script is \texttt{experiments/run\_llama4\_equalized.py}.
...
We acknowledge an adversarial reading in which sycophancy is effectively instructed deception with an extra reasoning step. We keep the semi-autonomous label because (i) the target behavior under the sycophancy prompt is \emph{agreement}, and the correction signal arises only on knowledge-conflict trials, so the refusal-count detector is not keying on a pre-specified deception mode; and (ii) the same system-prompt-induced-disposition pattern produces deceptive alignment concerns in the broader safety literature \cite{hubinger2019risks, greenblatt2024alignment}. This is a \emph{mechanistic} disambiguation: an adversarial reader who treats sycophancy as instructed-deception-with-one-extra-step cannot be experimentally falsified with current data. A system-prompt-only control without user-asserted false claims (limitation~(l)) is needed to cleanly isolate the disposition-source from the knowledge-conflict mechanism, and would be the decisive experimental test.
...
Three naive annotators independently classified 50 equalized transcripts (stratified across 7 targets; 22 truthful + 28 lying; seed=42) as TRUTH or LIE without knowledge of the ground-truth distribution. Annotators A/B/C received separate shuffle orders (seeds 43/44/45).

\textbf{Result.} The crowdworker baseline is \textbf{degenerate}: all three annotators defaulted almost entirely to TRUTH (A: 50/50; B: 48/50; C: 49/50); Fleiss'~$\kappa = 0.00$. Per-annotator accuracy: 44.0\% (Wilson 95\%~CI [31.2\%, 57.7\%]).

\begin{table}[h]
\centering
\caption{Human baseline (3 annotators) vs.\ automated rule, $n=50$ equalized sub-sample. $^\dag$From Table~\ref{tab:cross_family_panel} (7-target panel, $n{\approx}689$); not computed on the $n=50$ sub-sample.}
\label{tab:human_baseline}
\begin{tabular}{lrr}
\toprule
\textbf{Method} & \textbf{Accuracy} & \textbf{Wilson 95\% CI} \\
\midrule
Human Annotator A ($n{=}50$)     & 44.0\% & [31.2\%, 57.7\%] \\
Human Annotator B ($n{=}50$)     & 44.0\% & [31.2\%, 57.7\%] \\
Human Annotator C ($n{=}50$)     & 44.0\% & [31.2\%, 57.7\%] \\
Refusal-count rule ($n{=}50$)   & \textbf{76.0\%} & [62.6\%, 85.7\%] \\
\midrule
\emph{Pipeline same-family ($n{\approx}689$)} & \emph{74.3\%} & \emph{---} \\
\emph{Pipeline cross-family ($n{\approx}689$)$^\dag$} & \emph{64.7\%} & \emph{---} \\
\emph{Rule pooled-LOO ($n{\approx}689$)}       & \emph{80.1\%} & \emph{---} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Interpretation.} Degenerate $\kappa = 0.00$ reflects the equalized-condition design: lying-trial assistants (not instructed to lie) frequently produce corrections indistinguishable from truthful-trial responses, so the heuristic ``corrections~$\Rightarrow$~truthful'' systematically misfires. The $+$32\,pp gap between the rule and annotators measures performance over a \emph{naive unaided} all-TRUTH baseline; it does not reflect an intrinsic human-vs-machine capability difference. Annotators trained on the equalized design could in principle learn the rule.
...
As a partial response to the $n\geq 100$/3-annotator human ICC ask---which requires multi-week recruitment not tractable in a single revision---we computed a three-rater machine-ICC (Shrout--Fleiss (2,1), absolute-agreement) on 5 of 8 targets with triple-rater files available (Haiku, Llama~8B, Qwen~7B/14B/32B; pooled $n_{\mathrm{trials}}=495$) using Claude Haiku~4.5, Llama~3.3~70B Instruct, and Mistral Large~3 as the three raters. Pooled mean ICC(2,1)=\textbf{0.79}, ICC(2,k)=\textbf{0.89}; per-feature ICC(2,1): elaboration 0.95, specificity 0.94, confidence 0.91, defensiveness 0.77, consistency 0.37. The defensiveness jump from human ICC 0.114 to machine ICC 0.77 is the pattern expected when raters share systematic biases (overlapping training, similar rubrics) that humans do not, so this proxy does \emph{not} substitute for human ICC. Its informative content is subtractive: the Qwen~2.5~32B row collapses to ICC(2,1) 0.00--0.06 across all five features (reflecting the RLHF zero-refusal-marker collapse, Section~\ref{sec:qwen_scale_sweep}), which is expected given zero refusal markers in both conditions. Raw $n=20$ annotator files are released in supplementary material. This appendix section reports the machine proxy for completeness; the full $n=100$/3-annotator human ICC study is reported in Appendix~\ref{app:human_icc_n100}.

\section{Human ICC Study: $n=100$, 3 Annotators}
\label{app:human_icc_n100}

Three independent annotators rated $n=100$ AI conversation transcripts on five behavioural features (0--10 integer scale) via Prolific. Transcripts were stratified across 7 target models (50 truthful / 50 lying). Annotators were blind to ground-truth labels and to each other's ratings. Three attention-check items were embedded to verify engagement; all three annotators passed ($\leq 1$ failure allowed). Raw per-rater CSVs are released in supplementary material.

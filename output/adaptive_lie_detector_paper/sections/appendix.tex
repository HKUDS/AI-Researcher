...
\paragraph{Feature separability.} All five features exhibit very large separation: consistency ($d=4.70$), assertiveness ($d=4.58$), confidence ($d=4.56$), specificity ($d=4.43$), elaboration ($d=3.40$). 5-fold CV on 500 examples yields $100\% \pm 0\%$, reflecting engineered discriminability in the mock setting.
...
\toprule
\textbf{Regex Feature} & \textbf{Consist.} & \textbf{Specif.} & \textbf{Assert.} & \textbf{Confid.} & \textbf{Elab.} \\
\midrule
